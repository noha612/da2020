...
public class Mercator {
  ...
  public static CommonUtil.Coordinate projection(CommonUtil.Coordinate coor){
    yAxisProjection(coor.getX());
    xAxisProjection(coor.getY());
    return coor;
  }

  public static CommonUtil.Coordinate inverseProjection(CommonUtil.Coordinate coor){
    yAxisInverseProjection(coor.getX());
    xAxisInverseProjection(coor.getY());
    return coor;
  }

  private static double xAxisProjection(double input) {
    return Math.toRadians(input) * RADIUS_MAJOR + Math.toRadians(-0.001545);
  }

  private static double yAxisProjection(double input) {
    return Math.log(Math.tan(Math.PI / 4 + Math.toRadians(input) / 2)) * RADIUS_MAJOR;
  }

  private static double xAxisInverseProjection(double input) {
    return Math.toDegrees((input - Math.toRadians(-0.001545)) / RADIUS_MAJOR);
  }

  private static double yAxisInverseProjection(double input) {
